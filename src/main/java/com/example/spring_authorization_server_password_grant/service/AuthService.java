    ...
    public String getAccessTokenForRequest(HttpServletRequest request) {
        ...
        try {
            OAuth2AuthorizationCodeRequestAuthenticationToken requestAuthenticationToken =
                    new OAuth2AuthorizationCodeRequestAuthenticationToken(LOCAL_AUTHORIZATION_URI, authProperties.getClientId(),
                            principal, authProperties.getRedirectUri(), "STATE", Sets.newHashSet("openid", "read", "write"), null);
            AuthorizationServerSettings authorizationServerSettings = AuthorizationServerSettings.builder().issuer(authProperties.getIssuerUri()).build();
